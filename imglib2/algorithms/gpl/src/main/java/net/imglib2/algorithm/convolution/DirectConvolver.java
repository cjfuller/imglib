 ...
 * @param <OUT>
 */
public class DirectConvolver< T extends RealType< T >, O extends RealType< O >, K extends RealType< K >, IN extends RandomAccessibleInterval< T >, OUT extends RandomAccessibleInterval< O >, KERNEL extends RandomAccessibleInterval< K >> implements UnaryOperation< IN, OUT >
{
	...
	public OUT compute( IN input, OUT output )
	{
		...
		if ( m_kernel == null )
			throw new IllegalStateException( "Kernel not set in Direct Convolver" );

		final RandomAccess< T > srcRA = m_fac.create( input );

		final Cursor< K > kernelC = Views.iterable( m_kernel ).localizingCursor();
		...
		while ( resC.hasNext() )
		{
			...
			while ( kernelC.hasNext() )
			{
				...
				val += accessSrc.getRealDouble() * kernelC.get().getRealDouble();
			}
	...
	{
		return new DirectConvolver< T, O, K, IN, OUT, KERNEL >( m_kernel, m_fac );
	}
